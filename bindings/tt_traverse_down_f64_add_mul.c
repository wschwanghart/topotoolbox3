/*
  ...
  void traverse_down_f64_add_mul(float *output, float *input,
                                 ptrdiff_t *source,
                                 ptrdiff_t *target,
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  // [output] = tt_traverse_down_f64_add_mul(output, input, source, target)

  if (nrhs != 4) {
    mexErrMsgIdAndTxt("tt3:tt_traverse_down_f64_add_mul",
                      "Four inputs required");
  }

  if (!mxIsClass(prhs[0], "double")) {
    mexErrMsgIdAndTxt("tt3:tt_traverse_down_f64_add_mul",
                      "output argument must be of class double");
  }

  if (!mxIsClass(prhs[1], "double")) {
    mexErrMsgIdAndTxt("tt3:tt_traverse_down_f64_add_mul",
                      "input argument must be of class double");
  }
  ...
  if (nlhs != 1) {
    mexErrMsgIdAndTxt("tt3:tt_traverse_down_f64_add_mul", "One output required");
  }
  ...
  double *input = mxGetDoubles(prhs[1]);

  ptrdiff_t *source = (ptrdiff_t *)mxGetInt64s(prhs[2]);
  ptrdiff_t *target = (ptrdiff_t *)mxGetInt64s(prhs[3]);

  traverse_down_f64_add_mul(output, input, source, target, edge_count);
}
