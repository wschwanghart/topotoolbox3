...
function compileTask(~)
    ...
    %% Run cmake
    res = system(strcat(cmake," -S bindings/ -B build -DCMAKE_BUILD_TYPE=Release"), ...
        "LD_LIBRARY_PATH", ld_library_path) == 0 && ...
    system(strcat(cmake," --build build --config Release --verbose"), ...
        "LD_LIBRARY_PATH", ld_library_path) == 0 && ...
    system(strcat(cmake, ...
        " --install build --prefix toolbox/internal/mex --component Runtime"), ...
        "LD_LIBRARY_PATH", ld_library_path) == 0;
    if ~res
        error("libtopotoolbox compilation error");
    end
end
